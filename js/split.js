...
import { exportToPDF } from './replace.js'; // Reuse export
...
export function updateSplitModeUI(mode) {
  ...
  for (let i = 1; i <= mode; i++) {
    ...
    section.className = 'output-section bg-white p-4 rounded shadow-md';

    const textarea = document.createElement('textarea');
    textarea.id = `output${i}-text`;
    textarea.placeholder = `Kết quả chương ${i} sẽ xuất hiện ở đây...`;
    textarea.rows = 10;
    ...
    copyButton.className = 'bg-blue-500 hover:bg-blue-700 text-white py-1 px-2 rounded';
